...
int exportCarFileAtPath(NSString * carPath, NSString *outputDirectoryPath)
{
	NSError *error = nil;
	...
	for (NSString *key in [storage allRenditionNames])
	{
		...
		for( CUINamedImage *image in images )
		{
			...
			else
			{
				...
				printf("\t%s\n", [name UTF8String]);
                if(CGImageWriteToFile(cgImage, [outputDirectoryPath stringByAppendingPathComponent:name])){
                    sum++;
                }
			}
		}
